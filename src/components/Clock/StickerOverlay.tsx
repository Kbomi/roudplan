...
export default function StickerOverlay({ stickers, selectedId, onSelect, onUpdate, onDelete }: Props) {
      ...
      {stickers.map(s => (
        <div
          ...
          style={{
            ...
            border: selectedId === s.id ? '2px dashed #7F77DD' : '2px solid transparent',
            borderRadius: 6,
        ...
        >
          ...
          {selectedId === s.id && (
            <div
              ...
              onMouseDown={e => { e.stopPropagation(); handleResizeDown(e, s.id); }}
            />
          )}
        </div>
      ))}
