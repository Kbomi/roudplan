    ...
    <div style={{ display:'flex', flexDirection:'column', gap:6, maxHeight:280, overflowY:'auto', paddingRight:2 }}>
      {segments.map(seg => (
        <div key={seg.id}
          ...
          <div style={{ width:14, height:14, borderRadius:'50%', background:seg.color, flexShrink:0, border:'1px solid #e0e0e0' }} />
          <span style={{ fontSize:16, flexShrink:0 }}>{seg.emoji}</span>
          <span style={{ fontSize:13, flex:1, fontWeight:500, color:'#444', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            {seg.label}
          </span>
          <span style={{ fontSize:11, color:'#aaa', flexShrink:0 }}>
            {formatTime(seg.startHour, seg.startMinute)}~{formatTime(seg.endHour, seg.endMinute)}
          </span>
          <button style={iconBtn} title="수정" onClick={() => onEdit(seg)}>✏️</button>
          <button style={{ ...iconBtn, color:'#e24b4a' }} title="삭제" onClick={() => onDelete(seg.id)}>🗑️</button>
        </div>
      ))}
    </div>
  );
