...
export function generateComment(tab: TabType, segments: Segment[]): string {
  ...
  if (tab === 'baby_feed') {
    ...
    const sleepMins = segments
      ...
      .reduce((sum, s) => sum + totalMinutes(s.startHour, s.startMinute, s.endHour, s.endMinute), 0);
  ...
  const workMins = segments
    .filter(s => s.label.includes('일') || s.label.includes('공부') || s.emoji === '💼' || s.emoji === '📚')
    .reduce((sum, s) => sum + totalMinutes(s.startHour, s.startMinute, s.endHour, s.endMinute), 0);

  const leisureMins = segments
    .filter(s => s.label.includes('여가') || s.emoji === '🎮' || s.emoji === '🎬')
    .reduce((sum, s) => sum + totalMinutes(s.startHour, s.startMinute, s.endHour, s.endMinute), 0);

  const exerciseMins = segments
    .filter(s => s.label.includes('운동') || s.emoji === '🏃' || s.emoji === '💪')
    .reduce((sum, s) => sum + totalMinutes(s.startHour, s.startMinute, s.endHour, s.endMinute), 0);
